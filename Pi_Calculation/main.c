...
#error "unknown OpenCL SDK environment"
    ...
    float *result = (float*) calloc(local_size,sizeof(float)); /*the variable of partial sums*/
    ...
    /* Create buffers to hold the partial count */
     cl_mem result_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, local_size*sizeof(float), NULL,&ret);
    ...
    ret = clEnqueueReadBuffer(command_queue, result_buffer, CL_TRUE, 0,
       local_size*sizeof(float), result, 0, NULL, NULL);
    if(ret < 0) {
