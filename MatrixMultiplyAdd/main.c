...
int main()
{
    ...
    platforms = (cl_platform_id*) malloc(sizeof(cl_platform_id) * platformCount);
    // Get storage requirements for platform data
    if(platforms==NULL){
...
#error "unknown OpenCL SDK environment"
    ...
    kernel = clCreateKernel(program, "simpleMultiplyAdd", &ret);
    if (ret != CL_SUCCESS) {
    ...
    if(D==NULL){
    printf("Error in allocating memory for D\n");
    exit(1);
    }
    printf("Initial values of Matrix D elements:\n");
    ...
    /* Verify result */
    printf("Final values of Matrix D elements:\n");
    for (int i = 0; i < wD*hD; i++) {
      printf ("%f ", D[i]);
    }
    printf("\n");
